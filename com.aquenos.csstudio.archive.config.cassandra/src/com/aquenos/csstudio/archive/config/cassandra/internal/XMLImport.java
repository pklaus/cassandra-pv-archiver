...
public class XMLImport extends DefaultHandler
{
    ...
    private LinkedList<CompressionLevelConfig> compression_level_configs = 
    		new LinkedList<CompressionLevelConfig>();
	...
	public XMLImport(final String cassandra_hosts, final int cassandra_port,
		...
		config = new CassandraArchiveConfig(
				cassandra_hosts,
				cassandra_port,
				cassandra_keyspace,
                    ...
                    throws SAXException
    {
        ...
        else if (element.equals(TAG_COMPRESSION_LEVEL))
        {
        	...
        	{
        		throw new XMLImportException("Unexpected compression-level entry while in state " + state);
        	}
        	String name_attr = attributes.getValue("", "name");
        	if (name_attr == null)
        	{
        		throw new XMLImportException("Tag compression-level is missing name attribute.");
        	}
        	String compression_period_attr = attributes.getValue("", "compression-period");
        	String retention_period_attr = attributes.getValue("", "retention-period");
        	long compression_period = compression_period_attr == null ? 0 : Long.valueOf(compression_period_attr);
        	long retention_period = retention_period_attr == null ? 0 : Long.valueOf(retention_period_attr);
        	// We (possibly) do not know the channel name yet. However, we can set it
        	// later, when we are inserting the configuration.
        	compression_level_configs.add(
        			new CompressionLevelConfig(name_attr, null, compression_period, retention_period));
        }
    }
                    ...
                    throws SAXException
    {
        ...
        else if (element.equals(TAG_CHANNEL))
        {
            ...
            try
            {
                ...
                {
                	final String other_engine_name = config.getEngineName(other_channel);
                	final String other_group_name = config.getGroupName(other_channel);
                	...
                	if (steal_channels)
                	{
                    	System.out.format("Channel '%s/%s - %s' already found in '%s/%s', moved to this engine\n",
                                ...
                                other_engine_name, other_group_name);
                ...
                final SampleMode mode = new SampleMode(monitor, sample_value, period);
                final ChannelConfig channel = config.createChannel(group, name, mode);
                if (is_enabling)
                {
                	group = config.updateGroupEnablingChannel(group, channel.getName());
                }
                for (CompressionLevelConfig compression_level_config : compression_level_configs)
                {
                	config.createCompressionLevel(name, compression_level_config.getCompressionLevelName(), compression_level_config.getCompressionPeriod(), compression_level_config.getRetentionPeriod());
                }
            }
            catch (Exception ex)
