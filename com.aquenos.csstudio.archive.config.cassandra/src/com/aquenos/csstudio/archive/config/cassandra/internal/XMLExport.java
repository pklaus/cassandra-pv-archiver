...
public class XMLExport
{
    ...
    private void dumpChannel(final PrintStream out, final ChannelConfig channel)
    {
        ...
        if (mode.isMonitor())
        {
            ...
            out.println("      <scan/>");
        for (CompressionLevelConfig compressionLevelConfig : config.findCompressionLevelConfigs(channel.getName()))
        {
        	out.print("      <compression-level");
        	out.print(" name=\"" + compressionLevelConfig.getCompressionLevelName() + "\"");
        	long compression_period = compressionLevelConfig.getCompressionPeriod();
