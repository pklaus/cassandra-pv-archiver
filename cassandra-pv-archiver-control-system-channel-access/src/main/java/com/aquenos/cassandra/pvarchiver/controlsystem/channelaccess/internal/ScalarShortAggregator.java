...
/**
 * <p>
 * Aggregator for scalar short samples. This aggregator consumes samples of type
 * <code>SCALAR_SHORT</code> and produces samples of type
 ...
 */
public class ScalarShortAggregator extends AveragingSampleAggregator {
     ...
     *            aggregator.
     */
    public ScalarShortAggregator(long intervalStartTime, long intervalLength,
    ...
    protected void processSampleValue(ChannelAccessUdtSample sample,
        ...
        // a precondition.
        assert (sample.getType().equals(ChannelAccessSampleType.SCALAR_SHORT));
        short value = sampleValueAccess
                .deserializeShortValueColumn((ChannelAccessRawSample) sample);
        minimum = (short) Math.min(value, minimum);
        maximum = (short) Math.max(value, maximum);
