...
public class CassandraChannelConfig extends ChannelConfig {
     ...
     * @param sampleStore
     *            sample store for reading the last timestamp when requested.
     */
    public CassandraChannelConfig(String engineName, String groupName,
            String channelName, SampleMode sampleMode, SampleStore sampleStore) {
        super(channelName, sampleMode, null);
    ...
    public ITimestamp getLastSampleTime() {
        try {
            return sampleStore.getLastSampleTimestamp(0L, getName());
        } catch (ConnectionException e) {
            throw new RuntimeException(
                    "Database operation failed while trying to determine last sample timestamp: "
                            + e.getMessage(), e);
        }
    }
